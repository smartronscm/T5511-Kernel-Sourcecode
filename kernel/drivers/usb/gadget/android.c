...
static ssize_t
adb_name_store(struct device *pdev, struct device_attribute *attr,
			       const char *buff, size_t size)
{
	return snprintf(adb_name, PAGE_SIZE, "%s", buff);
}
...
/* TODO:Rajiv - revisit if this sysfs node is realy required */
static DEVICE_ATTR(adb_display_name, S_IRUGO | S_IWUSR, adb_name_show, adb_name_store);
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		ret = of_property_read_u8(pdev->dev.of_node,
				...
				&pdata->uicc_nluns);
	ret = of_property_read_string(pdev->dev.of_node, "adb_diaplay_name", (const char **)&adb_name);
