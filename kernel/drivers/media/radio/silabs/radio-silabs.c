...
static spinlock_t rs_lock;
static int rsstatus=1;
...
static int enable(struct silabs_fm_device *radio)
{
	...
	mutex_unlock(&radio->lock);
	retval = set_property(radio, REFCLK_FREQ_PROP, FM_set_work_frequency);
	if (retval < 0)
		FMDERR("%s: REFCLK_FREQ_PROP set error\n", __func__);

	retval = set_property(radio, REFCLK_PRESCALE_PROP, FM_divide_frequency);
	if (retval < 0)
		FMDERR("%s: REFCLK_PRESCALE_PROP set error\n", __func__);
...
static irqreturn_t silabs_fm_isr(int irq, void *dev_id)
{
	 ...
	 */
	FMDBG("FM INT fUNC!!!!\n");
	if(spin_trylock(&rs_lock))
	{
		if(rsstatus==fm_resume)
		{
...
static int silabs_fm_fops_open(struct file *file)
{
	...
	retval = clk_prepare_enable(radio->clk_ret);
	if(retval!=0)
	{
		FMDERR("\n\n===========clk_prepare_enable ERROR retval %d \n\n",retval);
...
static int silabs_fm_fops_release(struct file *file)
{
	...
	atomic_inc(&radio->users);
	clk_disable_unprepare(radio->clk_ret);
	return retval;
}
				...
				const struct i2c_device_id *id)
{
	...
	radio->clk_ret = clk_get(&client->dev, "ref_clk");

	if(radio->clk_ret==NULL)
	{
		FMDERR("\n\n=========== clk_get error \n\n");
		goto err_all;
	}

	FMDBG("fm driver probe success\n");
	return 0;
...
static int silabs_suspend(struct device *dev){

	spin_lock(&rs_lock);
	rsstatus=0;
	spin_unlock(&rs_lock);
	FMDBG("%s:fm has been suspended!!!!\n",__func__);
	return 0;
}
static int silabs_resume(struct device *dev){

	spin_lock(&rs_lock);
	rsstatus=1;
	spin_unlock(&rs_lock);
	FMDBG("%s:fm has been resumed!!!!\n",__func__);
	return 0;
}

static const struct dev_pm_ops silabs_pm_ops = {
	.suspend	= silabs_suspend,
	.resume     = silabs_resume,
};
