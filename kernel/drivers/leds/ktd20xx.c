...
#define BLINK_BRIGHTNESS

#define KTD_I2C_NAME			"ktd20xx"
#define MAX_BRIGHTNESS		(LED_FULL)
//#define MIN_BRIGHTNESS		(3)

//RGB current limit to 5mA   Imax = RGB_BRIGHTNESS_LIMIT*0.125ma
#define RGB_BRIGHTNESS_LIMIT (5)
...
#define DEFAULT_RAISE_AND_FALL_TIME (0x11)
...
static void ktd20xx_breath_leds(struct ktd_dev_data *pktd, enum ktd20xx_led which){
    ...
    //pr_info("blink_brightness :%d delay_on:%d delay_off:%d  \n",(int)blink_brightness,(int)delay_on,(int)delay_off);
    ...
    //pr_info("dute_value :%d \n",dute_value);
	// mode set---IC work when both SCL and SDA goes high 2X slower
	i2c_smbus_write_byte_data(client, CTRL_RESET_REG, 0x18);
...
static int ktd20xx_parse_dt(struct ktd_dev_data *pdata){	
	pdata->red_gpio_pin = of_get_named_gpio(pdata->i2c_client->dev.of_node, "ktd,gpio", 0);
	if (!gpio_is_valid(pdata->red_gpio_pin)) {
		dev_err(&pdata->i2c_client->dev, "gpio reset pin %d is invalid.\n",	pdata->red_gpio_pin);
		return -EINVAL;
	}
	//gpio_led = pdata->red_gpio_pin;
	return 0;
}
...
static int ktd20xx_red_gpio_init(struct ktd_dev_data *pdata){
	...
	ret = gpio_request(pdata->red_gpio_pin, "gpio");
	if (ret < 0) {
...
static ssize_t 
show_chipinfo(struct device *dev, struct device_attribute *attr, char *buf)
{
	/*struct ktd_dev_data *pdata = dev_get_drvdata(dev);
	int index = 0;
	u8 value = 0;
	char *buffer = buf;
	ssize_t lenth =0;
	i2c_smbus_write_byte_data(pdata->i2c_client,0x00,0x43);
	for(;index < 10; index ++){
        value = i2c_smbus_read_byte_data(pdata->i2c_client,index);
		lenth = snprintf(buffer, PAGE_SIZE, "reg%d :0x%x \n",index,value);
		buffer += lenth;
	}*/
    //i2c_smbus_write_byte_data(pdata->i2c_client,0x00,0x40);
	return snprintf(buf, PAGE_SIZE, "chipinfo:ktd2027\n");
}
DEVICE_ATTR(chipinfo,0444, show_chipinfo, NULL);
static int ktd20xx_probe(struct i2c_client *client, const struct i2c_device_id *id){
    ...
    device_create_file(&client->dev, &dev_attr_chipinfo);
