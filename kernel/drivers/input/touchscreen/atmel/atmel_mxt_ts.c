...
static struct mxt_data *data_info;
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	//mxt_power_init(struct mxt_data * data,bool on)(data, true); // power on --leibo
...
{
		return snprintf(buf, 17, "Atmel_MXT1664T3\n");
}
		...
		struct class_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%u.%u.%02X\n",
			 data_info->info->version >> 4, data_info->info->version & 0xf, data_info->info->build);
}
...
dev_info(&client->dev, INFODEBUG"%s: start irq thread!\n", 
	...
	error = create_touchscreen_info(&client->dev);
	if(error < 0){
